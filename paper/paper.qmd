...
The novel texts used for analysis were sourced from Project Gutenberg under the library `gutenbergr` [@rGuten] [@rgutenberg]. Data was generated, extracted and cleaned using the open-source statistical programming language R [@r], leveraging functions from `tidyverse` [@rTidyverse], `tidytext` [@rTidytext], `rmarkdown` [@rMark], `dplyr` [@rDplyr], `ggplot2` [@rGgplot2], `scales` [@rScales], `here` [@rHere], `wordcloud` [@rCloud], `tm` [@rTm] and `knitr` [@rKnitr].
...
#| label: fig-cloud
#| layout-ncol: 2
#| fig-cap: "Visualizations of Word Frequency when Combining All Stream of Consciousness Novels"
#| fig-subcap: ["One Word"] #Add "Two Words" and "Three Words" subsequently when uncovering the trends for the previous one


filtered_text <- combined_text[combined_text$n >= 300, ] #only select words with a frequency larger or equal to 300
